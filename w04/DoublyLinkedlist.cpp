...
void removeTail(d_List *&L)
{
    ...
    while (p->pNext != nullptr && p->pNext->pNext != nullptr) {
        p = p->pNext;
    }
...
void removeBefore(d_List *&L, int val)
{
    if (L == nullptr || L->p_head == nullptr || L->p_head->pNext == nullptr) {
        return;
    }
    d_NODE* p = L->p_head;
    if (p->key == val) {
...
void removeAfter(d_List *&L, int val)
{
    ...
    d_NODE* p = L->p_head;
    while (p->pNext != nullptr && p->key != val) {
        p = p->pNext;
    }
...
bool removeElement(d_List *&L, int key)
{
    ...
    d_NODE* p = L->p_head;
    while (p->pNext != nullptr && p->pNext->key != key) {
        p = p->pNext;
    }
    if (p->pNext == nullptr) {
        return false;
